...
const STORAGE_KEYS = {
  ...
  RESET_FLAG: 'chess_master_hub_v8_10k_reset_done',
};
...
export function notifyGemsUpdated(newGems: number, reason?: string) {
  if (typeof window !== 'undefined') {
    window.dispatchEvent(
      new CustomEvent('chess_gems_updated', {
        detail: { gems: newGems, reason },
      })
    );
  }
}
...
// Auto Reset Execution on Startup (Ensures all players start fresh with 10,000 coins & 10,000 gems)
function ensureInitialReset() {
  ...
  try {
    ...
    if (!isReset) {
      ...
      localStorage.setItem(STORAGE_KEYS.GEMS, '10000');
...
// Gems Core
export function getUserGems(): number {
  if (typeof window === 'undefined') return 10000;
  const val = localStorage.getItem(STORAGE_KEYS.GEMS);
  if (!val) {
    localStorage.setItem(STORAGE_KEYS.GEMS, '10000');
    return 10000;
  }
...
export function setUserGems(gems: number, reason = 'Direct update'): void {
  if (typeof window === 'undefined') return;
  const sanitized = Math.max(0, gems);
  localStorage.setItem(STORAGE_KEYS.GEMS, sanitized.toString());
  notifyGemsUpdated(sanitized, reason);
}

export function addGems(amount: number, reason = 'Reward added'): number {
  const current = getUserGems();
  const updated = current + amount;
  setUserGems(updated, reason);
  return updated;
}

export function spendGems(amount: number, reason = 'Purchase'): boolean {
  const current = getUserGems();
...
export function syncBalancesToBackend(gems?: number, coins?: number): void {
  if (typeof window === 'undefined') return;
  try {
    ...
    fetch('/api/currency/sync', {
      ...
      body: JSON.stringify({ coins: coins ?? getUserPoints(), gems: gems ?? getUserGems() }),
    }).catch(() => {});
  } catch {}
}
