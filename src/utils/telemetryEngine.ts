...
export const REGIONAL_EDGE_NODES: Record<string, { node: string; location: string; basePing: number; jitter: number }> = {
  IN: { node: 'IN-BOM-01', location: 'Mumbai, India', basePing: 18, jitter: 2.1 },
  US: { node: 'US-IAD-02', location: 'Virginia, USA', basePing: 24, jitter: 1.8 },
  GB: { node: 'EU-LHR-01', location: 'London, UK', basePing: 28, jitter: 2.4 },
  JP: { node: 'AP-NRT-01', location: 'Tokyo, Japan', basePing: 32, jitter: 1.5 },
  DE: { node: 'EU-FRA-01', location: 'Frankfurt, Germany', basePing: 22, jitter: 1.9 },
  BR: { node: 'SA-GRU-01', location: 'São Paulo, Brazil', basePing: 64, jitter: 4.8 },
  FR: { node: 'EU-CDG-01', location: 'Paris, France', basePing: 25, jitter: 2.2 },
  CA: { node: 'NA-YYZ-01', location: 'Toronto, Canada', basePing: 30, jitter: 2.0 },
  AU: { node: 'AP-SYD-01', location: 'Sydney, Australia', basePing: 78, jitter: 5.1 },
  KR: { node: 'AP-ICN-01', location: 'Seoul, S. Korea', basePing: 35, jitter: 1.7 },
  ES: { node: 'EU-MAD-01', location: 'Madrid, Spain', basePing: 38, jitter: 3.0 },
  IT: { node: 'EU-MXP-01', location: 'Milan, Italy', basePing: 34, jitter: 2.6 },
  AE: { node: 'ME-DXB-01', location: 'Dubai, UAE', basePing: 45, jitter: 3.4 },
  KE: { node: 'AF-NBO-01', location: 'Nairobi, Kenya', basePing: 92, jitter: 6.8 },
};
...
class TelemetryEngine {
  ...
  public getNetworkLatencyData(): { countryCode: string; node: string; location: string; pingMs: number; jitterMs: number; status: string }[] {
    return SUPPORTED_COUNTRIES.map((c) => {
      const edge = REGIONAL_EDGE_NODES[c.code] || REGIONAL_EDGE_NODES.IN;
      return {
        countryCode: c.code,
