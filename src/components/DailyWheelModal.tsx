...
export const DailyWheelModal: React.FC<DailyWheelModalProps> = ({
  ...
  const handleSpin = () => {
    ...
    setTimeout(() => {
      ...
      // Log transaction to Firestore & local coin ledger
      executeCoinTransaction({
        amount: targetSlice.points,
        type: 'daily_spin',
        description: `Daily Lucky Spin: ${targetSlice.label}${isGlobalJackpotHit ? ' (Jackpot Hit!)' : ''}`,
        metadata: {
          points: targetSlice.points,
          isJackpot: isGlobalJackpotHit,
        ...
        >
          ...
          <div className="px-4 sm:px-5 py-2.5 bg-black/50 border-b border-white/5 flex flex-wrap items-center justify-between gap-2">
            <div className="flex items-center gap-2 text-xs text-slate-300">
              ...
              <span className="font-semibold text-slate-200">Coins Balance:</span>
              <span className="font-extrabold text-amber-300 font-mono text-sm">
                {(userPoints ?? 0).toLocaleString()} Coins
              </span>
          ...
          <div className="p-4 sm:p-6 flex flex-col items-center justify-center relative overflow-hidden bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-slate-900 via-[#0d0f14] to-black min-h-[390px] overflow-y-auto">
              ...
              <motion.div
                ...
                className={`mt-4 p-4 rounded-2xl border-2 text-center shadow-2xl backdrop-blur-md max-w-md w-full ${
                  isJackpotWin
                    ...
                    : 'bg-gradient-to-r from-amber-500/25 via-emerald-500/25 to-cyan-500/25 border-amber-400 shadow-[0_0_30px_rgba(245,158,11,0.35)]'
                }`}
              >
                <div className="text-xs font-black text-amber-300 flex items-center justify-center gap-1.5">
                  ...
                  <span>
                    {isJackpotWin
                      ? '★ ULTRA GRAND 1M COINS JACKPOT WINNER ★'
                      : `DAILY REWARD WON — SEGMENT #${wonSlice.segmentNumber}!`}
                  </span>
                </div>
                <div className="text-xl sm:text-2xl font-black text-white mt-1 flex items-center justify-center gap-2">
                  <span>You won</span>
                  <span
                    className={`font-mono flex items-center gap-1 ${isJackpotWin ? 'text-yellow-300 text-2xl drop-shadow-[0_0_10px_#facc15]' : 'text-amber-300'}`}
                  >
                    <span>🟡</span>
                    <span>+{(wonSlice?.points ?? 0).toLocaleString()} Coins</span>
                  </span>
                  <span>!</span>
                </div>
                <p className="text-xs text-slate-200 mt-1.5 font-medium">
                  {isJackpotWin
                    ? '🏆 You hit the Guaranteed Global 1,000,000 Coins Annual Jackpot! Credited to your live top-bar wallet.'
                    : `🎉 Successfully credited to your top-bar Coins Wallet. New balance: ${(userPoints ?? 0).toLocaleString()} Coins.`}
                </p>
                <div className="mt-2 text-[11px] text-amber-300/90 font-bold bg-black/40 py-1 px-3 rounded-lg border border-amber-500/20 inline-block">
                  ⏱️ 1 Spin Per Day Used • Next Free Daily Spin in 24 Hours
                </div>
              </motion.div>
            )}
            ...
            <div className="mt-4 flex flex-col items-center gap-2 w-full max-w-xs">
              ...
              >
                ...
                <span>
                  {isSpinning
                    ...
                    : wheelStatus.canSpin
                    ? 'SPIN WHEEL FOR DAILY COINS!'
                    : `NEXT FREE SPIN IN (${countdownText})`}
                </span>
              </button>
              ...
              <div className="text-[11px] text-slate-400 text-center mt-1 flex items-center justify-center gap-1">
                ...
                <span>&amp;</span>
                <span className="text-cyan-400 font-bold">Daily Quests</span>
              </div>
            </div>
          </div>
          ...
          <div className="p-3.5 bg-[#0e1017] border-t border-white/10 flex flex-wrap items-center justify-between gap-2 text-xs">
            <div className="flex items-center gap-2 text-slate-400">
              ...
              <span>Coins Rule: 1 Free Spin Per Day. Connected directly to your user coins balance.</span>
            </div>
