...
export const ConnectFourBoard: React.FC<ConnectFourBoardProps> = ({ gameMode: initialMode = 'ai', onGameEnd }) => {
  ...
  const [gameMode, setGameMode] = useState<'pvp' | 'ai' | 'local'>(initialMode);
  ...
  const [aiPlayers, setAiPlayers] = useState<Record<'r' | 'y', boolean>>({
    r: initialMode === 'ai' ? false : false,
    y: initialMode === 'ai' ? true : false,
  });
  ...
  useEffect(() => {
    ...
    if (initialMode === 'ai') {
      ...
      setAiPlayers({ r: false, y: false });
    }
  }, [initialMode, userColor]);
