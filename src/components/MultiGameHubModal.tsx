...
export const MultiGameHubModal: React.FC<MultiGameHubModalProps> = ({
  ...
  const games: {
    ...
    },
    {
      id: 'business-empire',
      category: 'board',
      title: '20. Business Empire Online',
      subtitle: 'Build • Invest • Trade • Become a Tycoon',
      description: 'Property empire tycoon board game with live market trends, asset portfolio, trading, auctions, skyscrapers, chance cards, and 3D dice rolling!',
      badge: 'Board & Tycoon 👑',
      badgeColor: 'bg-amber-500/20 text-amber-300 border-amber-400/30',
      icon: <span className="text-2xl">👑</span>,
      gradient: 'from-amber-950/60 via-yellow-950/40 to-slate-950/70',
      borderColor: 'border-amber-500/50 hover:border-amber-400',
      actionText: 'Play Business Empire',
      actionIcon: <Play className="w-4 h-4 fill-current" />,
      actionBg: 'bg-gradient-to-r from-amber-500 via-yellow-500 to-amber-600 hover:from-amber-400 hover:to-yellow-400 text-slate-950 font-bold',
      onClick: () => {
        onClose();
        if (onSelectGame) onSelectGame('business');
      },
    },
  ];
      ...
      <div className="relative w-full max-w-5xl max-h-[90vh] flex flex-col bg-[#0e0c12] border border-[#f3ce6b]/30 rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.9)] overflow-hidden text-white">
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredGames.map((game) => (
            <div
              key={game.id}
              data-game-id={game.gameId || game.id}
              data-game-title={game.title}
              className={`game-card group relative flex flex-col justify-between p-5 rounded-2xl bg-gradient-to-b ${game.gradient} border ${game.borderColor} transition-all duration-200 hover:-translate-y-1 hover:shadow-xl backdrop-blur-md`}
            >
              <div>
                {/* Card Top Row */}
              ...
              <button
                ...
                data-game-title={game.title}
                onClick={() => {
                  GameEconomy.requestGameStart(game.gameId || game.id, game.title, () => {
                    game.onClick();
                  });
                }}
                className={`game-card w-full py-2.5 px-4 rounded-xl font-extrabold text-xs flex items-center justify-center gap-2 shadow-md transition active:scale-95 cursor-pointer ${game.actionBg}`}
              >
                {game.actionIcon}
        ...
        <div className="p-4 bg-black/60 border-t border-white/10 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-gray-400">
          <div className="flex items-center gap-2">
            ...
            <span>
              <strong>Wheel of Luck Catalog &amp; Matchmaking</strong> randomly spins among connected players for all 20 games!
            </span>
          </div>
          {onOpenWheelLobby ? (
            <button
